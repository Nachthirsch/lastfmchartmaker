...
<template>
  <div class="bg-yellow-50 text-black p-8 rounded-lg shadow-neobrutalism">
    <div class="mb-8 flex justify-between items-center">
      <div>
        <h2 class="text-3xl font-black uppercase mb-1 transform -rotate-1">TOP TRACKS</h2>
        <div class="w-32 h-2 bg-blue-500 transform rotate-1"></div>
      </div>
      <div class="flex gap-3">
        <div class="relative">
          <button @click="showCollageOptions = !showCollageOptions" class="bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded-md text-sm font-bold flex items-center transition transform hover:-translate-y-1 border-2 border-black shadow-neobrutalism-sm">
          ...
          <div v-if="showCollageOptions" class="absolute right-0 mt-2 w-56 bg-blue-100 rounded-md border-2 border-black shadow-neobrutalism-sm z-10 transform rotate-1">
            <div class="p-4">
              <p class="text-sm text-black font-black mb-2 uppercase">Collage Size</p>
              <div class="grid grid-cols-3 gap-2 mb-4">
                <button v-for="size in collageSizes" :key="size.value" @click="createTrackCollage(size.value)" class="text-xs py-1.5 px-2 rounded border-2 border-black font-bold transition" :class="[size.value === selectedCollageSize ? 'bg-green-400 text-black transform -rotate-1' : 'bg-white text-black hover:bg-gray-100 transform hover:-translate-y-0.5']">
                  {{ size.label }}
                </button>
              </div>

              <p class="text-sm text-black font-black mb-2 uppercase">Theme</p>
              <div class="flex gap-2 mb-4">
                <button @click="collageTheme = 'dark'" class="flex-1 text-xs py-1.5 px-2 rounded border-2 border-black font-bold transition" :class="[collageTheme === 'dark' ? 'bg-black text-white transform -rotate-1' : 'bg-white text-black hover:bg-gray-100 transform hover:-translate-y-0.5']">Dark</button>
                <button @click="collageTheme = 'light'" class="flex-1 text-xs py-1.5 px-2 rounded border-2 border-black font-bold transition" :class="[collageTheme === 'light' ? 'bg-yellow-300 text-black transform rotate-1' : 'bg-white text-black hover:bg-gray-100 transform hover:-translate-y-0.5']">Light</button>
              </div>

              <div class="flex items-center mb-4">
                <input type="checkbox" id="showTrackNames" v-model="showTrackNames" class="w-5 h-5 rounded bg-white border-2 border-black focus:ring-blue-500" />
              ...
              <button @click="createTrackCollage()" class="w-full mt-3 bg-blue-500 hover:bg-blue-600 text-white text-sm py-2 px-3 rounded border-2 border-black font-bold transition transform hover:-translate-y-1 shadow-neobrutalism-sm">Create Collage</button>
            </div>
          </div>
        </div>
        <button @click="$emit('close')" class="bg-red-400 hover:bg-red-500 text-white rounded-full p-3 border-2 border-black transition transform hover:rotate-12 shadow-neobrutalism-sm">
    ...
    <div class="mb-5 flex justify-between items-center bg-blue-100 p-3 rounded-md border-2 border-black transform -rotate-0.5 shadow-neobrutalism-sm">
      <div class="text-sm font-bold">{{ username }}'s top tracks • {{ timeRange }}</div>
      <div class="text-sm font-bold bg-orange-300 px-3 py-1 rounded-md border-2 border-black transform rotate-1">Total: {{ tracksCount }} tracks</div>
    </div>

    <!-- Tracks Table -->
    <div class="overflow-hidden bg-white rounded-lg border-2 border-black shadow-neobrutalism">
      <table class="min-w-full divide-y-2 divide-black">
        <thead class="bg-blue-400">
          <tr>
            <th scope="col" class="px-6 py-3 text-left text-sm font-black text-black uppercase tracking-wider">Rank</th>
            <th scope="col" class="px-6 py-3 text-left text-sm font-black text-black uppercase tracking-wider">Track</th>
            <th scope="col" class="px-6 py-3 text-left text-sm font-black text-black uppercase tracking-wider">Plays</th>
          </tr>
        </thead>
        <tbody class="bg-white divide-y divide-gray-300">
          <tr v-for="(track, index) in topTracks" :key="track.mbid || index" @click="showTrackDetails(track.name, track.artist.name)" class="cursor-pointer hover:bg-yellow-100 transition-colors duration-200">
            <td class="px-6 py-4 whitespace-nowrap text-sm font-black text-black">#{{ index + 1 }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-black">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="text-black font-bold">{{ track.name }}</div>
                  <div class="text-xs text-gray-700">{{ track.artist.name }}</div>

                  <!-- Track Tags (Fetch on demand) -->
                  <div class="mt-1">
                    <button @click.stop="fetchTrackTags(track)" v-if="!track.tags || track.tags.length === 0" class="text-xs text-blue-600 font-bold hover:underline border-b-2 border-blue-500">View Tags</button>
                    <div v-else class="flex flex-wrap gap-1 mt-1">
                      <span v-for="(tag, tagIndex) in track.tags" :key="tagIndex" class="text-xs px-2 py-0.5 bg-purple-400 rounded border border-black text-black font-bold transform rotate-1">
                        {{ tag.name }}
                      </span>
                    </div>
                  </div>
    ...
    <div class="mt-8 flex justify-end">
      <button @click="$emit('close')" class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-md text-lg font-black transition transform hover:-translate-y-1 border-2 border-black shadow-neobrutalism-sm">Back to Overview</button>
    </div>
  </div>
</template>
