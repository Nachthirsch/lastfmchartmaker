  ...
  <div class="flex flex-col">
    <div class="flex justify-between items-center mb-2">
      ...
      <div class="flex gap-2">
        <button @click="$emit('view-all')" class="text-white bg-purple-600 hover:bg-purple-700 transition px-3 py-1 rounded-md text-sm flex items-center">
          <span class="mr-1">View All</span>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
            <path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/>
          </svg>
        </button>
        <button @click="$emit('share')" class="text-white bg-purple-600 hover:bg-purple-700 transition px-3 py-1 rounded-md text-sm flex items-center">
    ...
    <div v-if="topArtist" 
        @click="$emit('show-details', topArtist.name)"
    ...
    <div class="bg-gray-900 rounded-lg overflow-hidden shadow-md">
      <div v-for="(artist, index) in otherArtists" 
          :key="artist.mbid || index" 
          @click="$emit('show-details', artist.name)"
